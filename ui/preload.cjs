...
contextBridge.exposeInMainWorld('api', {
  ...
  setChannelOutput: (key, deviceId) => ipcRenderer.invoke('set-channel-output', key, deviceId),

  getIcon:  (exePath) => ipcRenderer.invoke('get-icon', exePath),

  getTestSigning:  () => ipcRenderer.invoke('get-testsigning'),
  setTestSigning:  () => ipcRenderer.invoke('set-testsigning'),

  installDriver:   () => ipcRenderer.invoke('install-driver'),
  uninstallDriver: () => ipcRenderer.invoke('uninstall-driver'),
  onDriverLog:  (cb) => ipcRenderer.on('driver-log',  (_e, line)   => cb(line)),
  onDriverDone: (cb) => ipcRenderer.on('driver-done', (_e, result) => cb(result)),
